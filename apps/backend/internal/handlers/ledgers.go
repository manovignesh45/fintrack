...
func (h *AccountHandler) GetLedgers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeInternalError(w, err, "Failed to fetch ledgers")
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&l.ID, &l.UserID, &l.Name, &l.CreatedAt, &l.UpdatedAt); err != nil {
			writeInternalError(w, err, "Failed to parse ledgers")
			return
		}
...
func (h *AccountHandler) CreateLedger(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeInternalError(w, err, "Failed to create ledger (name might be taken)")
		return
	}
